...
def add_image():
    ...
    file_path = filedialog.askopenfilename(
        initialdir="./petCTimagesBMP")
    original_image = Image.open(file_path)
    # Update dimensions of canvas to match the dimensions of the selected image
...
def apply_segmentation(segmentation_method):
    ...
    if segmentation_method == "kMeans":
        ...
        segment_path = os.path.join(output_dir, 'reGrowSegmentedImages/', f'{image_name}_region_growing.bmp')
...
def select_ksize(ksize_str):
    ...
    try:
        ...
        size = 7  # Set a default odd value
    return size
...
k_means_clustering_button = tk.Button(right_frame, text="Apply k-Means Clustering",
                                          command=lambda: apply_segmentation("kMeans"))
k_means_clustering_button.pack(pady=(30, 0))

# Region Growing Button
region_growing_button = tk.Button(right_frame, text="Apply Region Growing", command=lambda: apply_segmentation("regrow"))
region_growing_button.pack(pady=(0, 30))

# Select Segment 1 Button
select_segment1_button = tk.Button(right_frame, text="Select Segment 1", command= add_segment1)
select_segment1_button.pack()

# Select Segment 2 Button
select_segment2_button = tk.Button(right_frame, text="Select Segment 2", command= add_segment2)
select_segment2_button.pack()


# Dice coefficient button
dice_coefficient_button = tk.Button(right_frame, text="Calculate Dice Coefficient", command=lambda: calculateDiceCoefficient(segment1_path, segment2_path))
